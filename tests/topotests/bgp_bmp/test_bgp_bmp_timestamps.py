...
def test_peerup_timestamp_reestablish():
    ...
    events are >= 5s apart, so a >= 2s assertion threshold is safe at the
    1-second BMP timestamp granularity.
    """
    tgen = get_topogen()
...
def test_prepolicy_reannouncement_timestamp():
    ...
    record the timestamp of its pre-policy update, then after a wall-clock
    pause change ONLY the route-map contents (a different community).
    Route-map update processing re-advertises the prefix as an
    implicit-replace UPDATE -- no withdraw -- so r1ts modifies its existing
    Adj-RIB-In entry in place.  The new pre-policy update must carry the
    reception time of the re-announcement, not the frozen timestamp of the
    ...
    # Ensure a measurable wall-clock gap between the two receptions.
    time.sleep(3)
    ...
    # UPDATE (no withdraw), after the route-map update delay timer.
    tgen.gears["r2ts"].vtysh_cmd(
    ...
    assert delta >= 2.0, (
        ...
        "{}, re-announcement at {}, delta {:.6f}s despite a 3s wall-clock "
        "gap -- bgpd reused the Adj-RIB-In entry's original reception "
        "time".format(
            WATCHED_PREFIX,
            second["seq"],
            first["seq"],
...
def test_prepolicy_microsecond_timestamps():
    ...
    updates >= 2s apart (its 3s wall-clock sleep, not the route-map
    delay timer).  Drive three further re-announcements by
